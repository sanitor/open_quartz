...
- **Visible FPS accounting** — derive Renderer delivery FPS from `requestVideoFrameCallback().presentedFrames` deltas rather than callback invocation count; WebView2 batches multiple presented frames into callbacks, so the old counter underreported an actual ~60 FPS stream as 20–40 FPS.
