...
import { BrowserHost } from '../../src/sdk/internal/BrowserHost';
...
describe('BrowserHost worker transport', () => {
  beforeEach(() => {
  ...
  it('projects worker deliveries and closes without accepting more requests', async () => {
    const onFrame = vi.fn();
    const runtime = new BrowserHost({ onFrame });
    const canvas = {
  ...
  it('reports Renderer presentation only after drawing a mirror canvas', async () => {
    ...
    const runtime = new BrowserHost({ onRendererPresented });
