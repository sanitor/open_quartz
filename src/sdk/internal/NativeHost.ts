...
type NativeResourceIntent =
  ...
  | { type: 'upload-image'; nodeId: string; key: string; source: { kind: 'encoded'; source: string } | { kind: 'raw'; source: string; format?: string; width?: number; height?: number } }
  | { type: 'remove-image'; nodeId: string }
  | {
      type: 'load-onnx';
      nodeId: string;
      key: string;
      ...
      iouThreshold: number;
      download?: { modelId: string; url: string; expectedSize: number; sha256: string };
    }
  | { type: 'unload-onnx'; nodeId: string };

type NativeResourcePlan = {
  graph: HostGraphSnapshot;
  intents: NativeResourceIntent[];
};
...
export class NativeHost {
  ...
  async setGraph(nodes: Node<ShaderNodeData>[], edges: Edge[]): Promise<number> {
    const plan = await this.planResourceIntents(nodes, edges);
    const revision = await this.invoke<number>('native_gpu_set_graph', {
      graphJson: JSON.stringify(plan.graph),
    });
    await this.applyResourceIntents(plan.intents);
  ...
  private async planResourceIntents(
    nodes: Node<ShaderNodeData>[],
    edges: Edge[],
  ): Promise<NativeResourcePlan> {
    return JSON.parse(await this.invoke<string>('native_host_resource_intents', {
      requestJson: JSON.stringify({
        host: 'native',
        previousGraph: this.lastResourceGraph,
        graph: cloneHostGraph(nodes, edges),
      }),
    })) as NativeResourcePlan;
  }

  private async applyResourceIntents(intents: NativeResourceIntent[]): Promise<void> {
    for (const intent of intents) {
      switch (intent.type) {
        case 'detach-video':
          await this.detachVideo(intent.nodeId);
          break;
        case 'remove-image':
          await this.removeTexture(intent.nodeId);
          break;
        case 'unload-onnx':
          await this.unloadOnnxModel(intent.nodeId);
          break;
        case 'attach-video': {
          const info = await this.attachVideo(
            intent.nodeId,
            intent.kind,
            intent.source,
            intent.looping,
          ...
          this.callbacks.onOutputSize?.(intent.nodeId, info.width, info.height);
          break;
        }
        case 'upload-image': {
          const image = intent.source.kind === 'encoded'
            ? await decodeImageSource(intent.source.source)
            : decodeRawRgba(
                await fetchResourceBytes(intent.source.source),
                intent.source.width,
                intent.source.height,
                intent.source.format,
              );
          await this.uploadImage(intent.nodeId, image.rgba, image.width, image.height);
          break;
        }
        case 'load-onnx': {
          if (intent.download) {
            await this.invoke<string>('download_model', {
              modelId: intent.download.modelId,
              url: intent.download.url,
              expectedSize: intent.download.expectedSize,
              sha256: intent.download.sha256,
            });
          }
          const info = await this.loadOnnxModel(intent.nodeId, intent.modelId, intent.task, {
            modelPath: intent.modelPath,
            targetSize: intent.targetSize,
            scoreThreshold: intent.scoreThreshold,
            iouThreshold: intent.iouThreshold,
          });
          this.callbacks.onBackendDetected?.(intent.nodeId, 'native');
          this.callbacks.onNativeBackendDetected?.(intent.nodeId, info.backend);
          break;
        }
      }
    }
