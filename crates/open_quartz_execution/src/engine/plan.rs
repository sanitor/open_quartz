...
use open_quartz_schema::{DataType, Edge, InputMode, NodeType, ProjectNode};
use crate::wgsl::{
...
fn target_for(node: &ProjectNode, default_width: u32, default_height: u32) -> Option<TargetSpec> {
    ...
    let float = matches!(
        ...
        Some(open_quartz_schema::FramebufferFormat::Rgba32f)
            | Some(open_quartz_schema::FramebufferFormat::Rg32f)
            | Some(open_quartz_schema::FramebufferFormat::R32f)
    );
    Some(TargetSpec {
        width,
        height,
