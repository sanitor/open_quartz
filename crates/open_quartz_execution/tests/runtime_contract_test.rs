use open_quartz_execution::engine::{ExecutionCommand, ExecutionPlan, GpuFacade};
...
impl open_quartz_execution::runtime::HostBackend for RecordingBackend {
    ...
    fn register_resource(
        ...
        handle: u64,
    ) -> Result<(), open_quartz_schema::SdkError> {
    ...
    fn present(&mut self, _set: &PresentationSet) -> Result<(), open_quartz_schema::SdkError> {
        Ok(())
    }
}
...
use open_quartz_execution::runtime::{
    ...
    PresentationSet, ResourceDescriptor, RuntimeCapabilities, RuntimeFrameInput, Viewport,
};
...
impl HostBackend for FailingRemoveBackend {
    ...
    fn remove_resource(&mut self, _: &str, _: u64) -> Result<(), open_quartz_schema::SdkError> {
        Err(open_quartz_schema::SdkError::new(
            open_quartz_schema::SdkErrorCode::InvalidResource,
            "release failed",
        ))
    }
    fn present(&mut self, _: &PresentationSet) -> Result<(), open_quartz_schema::SdkError> {
        Ok(())
    }
}
...
fn canonical_runtime_is_send_and_selects_native_video_bindings() {
    ...
    let mut browser = PlayerHost::new(RuntimeCapabilities { data_paths: vec![] });
    ...
    let mut native = PlayerHost::new_native(RuntimeCapabilities { data_paths: vec![] });
