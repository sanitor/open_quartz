...
async fn output() -> open_quartz::gpu::GpuOutputHandle {
    ...
    let adapter = instance
        ...
        .await
        .expect("a fallback GPU adapter is required for presenter tests");
    ...
    open_quartz::gpu::GpuOutputHandle {
...
fn frame(output: &open_quartz::gpu::GpuOutputHandle, number: u64) -> GpuPresentationFrame {
...
fn latest_frame_mailbox_replaces_slow_presenter_work() {
    pollster::block_on(async {
        ...
        assert_eq!(
            mailbox.stats(),
            open_quartz::gpu::PresentationQueueStats {
                submitted: 3,
                replaced: 2,
                consumed: 1,
            }
