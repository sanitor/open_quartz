...
use open_quartz::gpu::{GpuBackend, GpuExecutor, GpuOutputHandle, TextureFormat};
use open_quartz::media::{
...
async fn output() -> (Arc<GpuBackend>, GpuOutputHandle) {
    ...
    let adapter = instance
        ...
        .await
        .expect("a fallback GPU adapter is required for media tests");
