use open_quartz::wgsl::parse_shader;
...
@fragment fn main(@location(0) uv: vec2f) -> @location(0) vec4f {
    ...
    assert!(parsed
        .inputs
        .iter()
        .all(|port| port.data_type == open_quartz::types::DataType::Float));
}
